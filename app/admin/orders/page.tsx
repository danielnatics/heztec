...
export default function AdminOrdersPage() {
  ...
  const saveOrderEdits = async () => {
    ...
    try {
      ...
      setOrders(
        orders.map((o) =>
          o.id === editingOrder.id ? { ...o, ...editingOrder } : o,
        ),
      );
  ...
  const renderStatusBadge = (status: string) => {
    if (status === "pending_verification")
      return (
        <span className="px-2 py-1 bg-amber-50 text-amber-700 text-[10px] font-black uppercase rounded-md border border-amber-200">
          <Clock size={12} className="inline mr-1" /> Pending
        </span>
      );
    if (status === "confirmed")
      return (
        <span className="px-2 py-1 bg-blue-50 text-blue-700 text-[10px] font-black uppercase rounded-md border border-blue-200">
          <CheckCircle2 size={12} className="inline mr-1" /> Confirmed
        </span>
      );
    if (status === "completed")
      return (
    ...
    if (status === "cancelled")
      return (
        <span className="px-2 py-1 bg-red-50 text-red-700 text-[10px] font-black uppercase rounded-md border border-red-200">
          <XCircle size={12} className="inline mr-1" /> Cancelled
        </span>
      );
    return null;
  };
                  ...
                  <div className="space-y-3 pt-2">
                    ...
                    <div className="flex gap-2 pt-2">
                      <button
                        ...
                        className="bg-slate-800 text-white text-xs px-3 py-1.5 rounded flex items-center gap-1 font-bold"
                      >
                        {updatingId === order.id ? (
                ...
                <div className="flex gap-2">
                  ...
                  {order.payment_status === "pending_verification" && (
                    <button
                      ...
                      className="flex-1 bg-slate-800 hover:bg-slate-900 text-white text-xs font-bold py-2.5 rounded-lg flex justify-center items-center gap-2"
                    >
                      {updatingId === order.id ? (
                  ...
                  {order.payment_status === "confirmed" && (
                    <button
                      ...
                      disabled={updatingId === order.id}
                      className="flex-1 bg-green-600 hover:bg-green-700 text-white text-xs font-bold py-2.5 rounded-lg flex justify-center items-center gap-2"
                    >
                      {updatingId === order.id ? (
                        <Loader2 size={14} className="animate-spin" />
                      ) : (
                  ...
                  {(order.payment_status === "pending_verification" ||
                    ...
                    <button
                      ...
                      disabled={updatingId === order.id}
                      className="px-3 border border-red-200 text-red-600 hover:bg-red-50 text-xs font-bold py-2.5 rounded-lg"
                      title="Cancel Order"
                    >
                      <XCircle size={16} />
                    </button>
